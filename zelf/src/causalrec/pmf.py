...
@jit(float32[:](float32[:, :], float32[:, :], int32[:], int32[:]), nopython=True)
def _inner(beta, theta, rows, cols):
    n_ratings = rows.size
    n_components, n_users = theta.shape
    data = np.empty(n_ratings, dtype=np.float32)
    for i in range(n_ratings):
        ...
        for j in range(n_components):
            data[i] += beta[rows[i], j] * theta[j, cols[i]]
    return data
...
def generate_replicated_data(self):
    # Simulate user and item latent factors from their respective Gamma distributions
    simulated_users = np.random.gamma(self.gamma_t, 1./self.rho_t)
    simulated_items = np.random.gamma(self.gamma_b, 1./self.rho_b).T  # Transpose to align dimensions

    # Calculate the expected rate matrix for the Poisson distribution
    rate_matrix = simulated_users @ simulated_items

    # Sample from the Poisson distribution to generate new data
    replicated_data = np.random.poisson(rate_matrix)
    return sparse.csr_matrix(replicated_data)