...
def load_data_rating(path, columns=[0, 1, 2], test_size=0.1, sep="\t"):
    df = pd.read_csv(path, sep=sep, header=None, names=['userId', 'songId', 'rating'], usecols=columns, engine="python")
    ...
    return train_matrix.todok(), test_matrix.todok(), max_user_id + 1, max_item_id + 1

train, test, user, item = load_data_rating('C:/Users/Sten Stokroos/Desktop/zelf/neural_collaborative_filtering/Data/ml-1m.train.rating', columns=[0, 1, 2], test_size=0.1, sep="\t")
