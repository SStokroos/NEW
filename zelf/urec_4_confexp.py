...
class UAutoRec4confexp():
    ...
    def build_network(self, hidden_neuron=500):
        ...
        # Confounder and exposure weight matrices
        W_conf = tf.Variable(tf.random.normal([self.num_item, self.num_item], stddev=0.01))
        W_exp = tf.Variable(tf.random.normal([self.num_item, self.num_item], stddev=0.01))
        ...
        # Adding weighted confounders and exposures after reconstruction
        self.layer_2 += tf.matmul(W_conf, self.confounder_matrix)
        self.layer_2 += tf.matmul(W_exp, self.exposure_matrix)
        
        # Loss function
        self.loss = tf.reduce_mean(tf.square(
            tf.norm(tf.multiply((self.rating_matrix - self.layer_2), self.rating_matrix_mask)))) + self.reg_rate * (
        tf.square(tf.norm(W)) + tf.square(tf.norm(V)) + tf.square(tf.norm(W_conf)) + tf.square(tf.norm(W_exp)))
