...
# class UAutoRec():
#     def __init__(self, sess, num_user, num_item, learning_rate=0.001, reg_rate=0.1, epoch=50, batch_size=200,
...
#         # Adding confounders after reconstruction
#         self.layer_2 += self.confounder_matrix
...
#     final_model = UAutoRec(sess, user, item, learning_rate=0.001, reg_rate=0.1, epoch=50, batch_size=500, verbose=True)
#     final_model.build_network(hidden_neuron=500)
...
class UAutoRec():
    ...
    def build_network(self, hidden_neuron=500):
        ...
        # Confounder weight matrix
        W_conf = tf.Variable(tf.random.normal([self.num_item, self.num_item], stddev=0.01))
        ...
        # Adding weighted confounders after reconstruction
        self.layer_2 += tf.matmul(W_conf, self.confounder_matrix)
        
        # Loss function
        self.loss = tf.reduce_mean(tf.square(
            tf.norm(tf.multiply((self.rating_matrix - self.layer_2), self.rating_matrix_mask)))) + self.reg_rate * (
        tf.square(tf.norm(W)) + tf.square(tf.norm(V)) + tf.square(tf.norm(W_conf)))
