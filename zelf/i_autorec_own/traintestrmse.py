...
class IAutoRec():
    ...
    def execute(self, train_data, val_data, test_data):
        ...
        with tqdm(total=self.epochs, desc="Training", unit="epoch") as pbar:
            for epoch in range(self.epochs):
                ...
                if (epoch) % self.T == 0:
                    test_rmse, _ = self.test(test_data)
...
train, val, test, user, item = load_data_rating('ml', columns=[0, 1, 2], sep="\t")
