...
class UAutoRec():
    ...
    def build_network(self, hidden_neuron=500):
        ...
        # Concatenate along the first axis (number of items)
        combined_input = tf.concat([self.rating_matrix, self.confounder_matrix, self.exposure_matrix], axis=0)
        
        # Update dimensions for the combined input
        input_dim = self.num_item * 3  # Assuming num_item is the same for rating, confounder, and exposure
        
        V_combined = tf.Variable(tf.random.normal([hidden_neuron, input_dim], stddev=0.01))
        mu_combined = tf.Variable(tf.random.normal([hidden_neuron], stddev=0.01))
        
        # Apply the sigmoid activation
        layer_1 = tf.sigmoid(tf.expand_dims(mu_combined, 1) + tf.matmul(V_combined, combined_input))
        ...
        self.loss = tf.reduce_mean(tf.square(
            ...
            tf.square(tf.norm(W)) + tf.square(tf.norm(V_combined)))
...
CAUSEFIT_DIR = 'C:/Users/Sten Stokroos/Desktop/Thesis2.0/zelf/dat/out/ml_wg_0.2split'
