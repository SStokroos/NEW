...
if __name__ == '__main__':
    ...
    # Checks
    # num_users, num_items = substitute_values.shape


    # example_user_index = 0  # Example, needs to be replaced with actual use case
    # example_item_index = num_items - 1  # Last item index based on zero-based indexing

    # # Verify that indices are within bounds
    # if example_user_index >= num_users or example_user_index < 0:
    #     print("User index is out of bounds!")
    # if example_item_index >= num_items or example_item_index < 0:
    #     print("Item index is out of bounds!")

    # # Print example confounder values to check outputs
    # print("Example confounder values for user:", example_user_index, "and item:", example_item_index)
    # print(substitute_values[example_user_index, example_item_index])



    (hits, ndcgs) = evaluate_model(model, testRatings, testNegatives, topK, evaluation_threads) #, substitute_values)
    ...
    for epoch in tqdm(range(epochs)): #used to be xrange
        t1 = time()
        # # Generate training instances
        # user_input, item_input, labels = get_train_instances(train, num_negatives)
        print("mode wll be fitted here")
        # Training        
        # hist = model.fit([user_input, item_input], #input 
        #                  labels, # labels 
        #                  batch_size=batch_size, epochs=1, verbose=1, shuffle=True)

        # Assuming you have prepared 'substitute_values' which is the full matrix of substitute confounders
        user_input, item_input, labels= get_train_instances(train, num_negatives)   #, substitute_values), could add add subsitutes
        hist = model.fit([user_input, item_input], labels, batch_size=batch_size, epochs=epochs, verbose=1, shuffle=True) #could add substitues 
