...
# # os.environ['THEANO_FLAGS'] = 'cxx=g++'

# import keras
# from keras import backend as K
# print("Using backend:", K.backend())
# import theano
# # print("Theano C++ compiler:", theano.config.cxx)
...
def get_model(num_users, num_items, latent_dim, regs=[0,0]):
    ...
    MF_Embedding_User = Embedding(input_dim=num_users, output_dim=latent_dim, name='user_embedding',
                              embeddings_initializer=lambda shape, dtype: init_normal(shape, dtype=dtype),
...
#         labels.append(1)
#         # negative instances
#         for t in range(num_negatives): #used to be xrange
...
#             j = np.random.randint(num_items)
#             # Ensure that the negative instance is not in the training data
#             while (u, j) in train:
#                 j = np.random.randint(num_items)
#             user_input.append(u)
#             item_input.append(j)
...
def get_train_instances(train, num_negatives):
    print("Starting to generate training instances...")
    user_input, item_input, labels = [], [], []
    num_users, num_items = train.shape
...
if __name__ == '__main__':
    ...
    if learner.lower() == "adagrad": 
        model.compile(optimizer=Adagrad(learning_rate=learning_rate), loss='binary_crossentropy') #learning_rate used to be lr
